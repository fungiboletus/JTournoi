...
class Ihm {
    ...
    public static ArrayList<TypeEpreuve> recolterEpreuves() throws Exception {
        ...
        System.out
                .print("Entrez le numéro des épreuves choisies séparés par des espaces :");
        ...
        for (int i = 0; i < epreuvesId.size(); i++) {
            ...
            for (int j = 0; j < Stock.getTypesEpreuves().size(); j++) {
                if (Stock.getTypesEpreuves().get(j).getId() == epreuvesId
                        .get(i))
                    contient = true;
            }
            if (!contient)
        ...
        for (int i = 0; i < epreuvesId.size(); i++) {
            for (int j = 0; j < Stock.getTypesEpreuves().size(); j++) {
                if (Stock.getTypesEpreuves().get(j).getId() == epreuvesId
                        .get(i))
                    typeEpreuves.add(Stock.getTypesEpreuves().get(j));
            }
        }
        return typeEpreuves;
    }
    ...
    public static ArrayList<Equipe> recolterEquipes() throws Exception {
        ...
        // Phase de récolte des épreuves
        System.out
                .print("Entrez le numéro d'identifiant des équipes que vous incluez, séparés par des espaces :");
        s = new Scanner(System.in);
        String lue = s.nextLine();
    ...
    /**
     * Permet de découper une chaine selon les espaces et de renvoyer une liste
     * d'entiers
     * 
     * @return id Les entiers initialement séparés par des espaces dans la
     *         chaine
     * @param lue
     *            La chaine initiale
     */
    ...
    /**
     * Permet de découper une chaine selon les espaces et de renvoyer des
     * chaines découpées
     * 
     * @return id Les entiers initialement séparés par des espaces dans la
     *         chaine
    ...
    public Joueur getJoueurByName(String nom, Equipe equipe) {
        for (int i = 0; i < equipe.getNombreDeJoueurs(); i++) {
            if (equipe.getJoueur(i).getNom().equals(nom))
     ...
     */
    public ArrayList<Joueur> getJoueursCompetents(Equipe equipe,
            TypeEpreuve type) {
    ...
    public ArrayList<Tournoi> getTournoi(Arbitre a) {
        ...
        for (int i = 0; i < Moteur.listeTournoi.size(); i++) {
            // Pour chaque match
            try {
                for (int j = 0; j < Moteur.getCurrentMatch(
                        Moteur.listeTournoi.get(i)).size(); j++) {
                    if (Moteur.getCurrentMatch(Moteur.listeTournoi.get(i)).get(
                            j).getArbitre().equals(a)) {
     ...
     * 
     * @param message
     *            le message à afficher avant la demande.
