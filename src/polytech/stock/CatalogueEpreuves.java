...
/**
 * @author Antoine Pultier Le catalogue d'épreuves est un fichier texte au
 *         format CSV.
 * 
	...
	public static List<TypeEpreuve> recupererTypesEpreuves() {
		...
		try {
			...
			while ((ligne = reader.readNext()) != null) {
				if (ligne.length < 6) {
					System.out
							.println("Une ligne a été ignorée par manque d'informations");
				} else {
					epreuves.add(new TypeEpreuve(Integer.parseInt(ligne[0]),
							ligne[1], ligne[2], Integer.parseInt(ligne[3]),
							Integer.parseInt(ligne[4]), Integer
									.parseInt(ligne[5]) == 0));
				}
			}
		} catch (Exception e) {
			System.out
					.println("Erreur de chargement du catalogue d'épreuves : "
							+ e.getMessage());
		}
