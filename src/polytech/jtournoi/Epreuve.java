 ...
 * @ param : type d'epreuve, liste d'équipe Création et
 * réalisation d'une épreuve avec les équipes passée en paramètre
 * 
 */
public class Epreuve extends TupleAvecID {
	 ...
	 * 
	 * @param Hashmap
	 *            des équipes et des joueurs défini pour chaque équipe
	 * @param te
	 *            Type de l'épreuve
	...
	public Epreuve(ArrayList<Equipe> equipe, TypeEpreuve te)
		...
		// si il n'y a qu'eune seul équipe elle est vainqueur (ne devraiit pas
		// se présenter)
		if (equipe.size() == 1) {
	...
	public void tour() throws nbrArbitreInsufisantException {
		...
		// boucle uniquement pour le premier tour pour ramener le nombre
		// d'équipe a une puissance de 2
		for (int i = 0; i < indiceStandBy; i++) {
			vainqueurEquipe.add(current.get(i));
		}
		// on réalise autant de match qu'il faut pour ramener le nombre d'équipe
		// a une puissance de deux
		int j = indiceStandBy;
		// si le nombre est une puissance de deux alors tout les matchs sont
		// réalisé
		int inc = 0;
	...
	private void remove(Match m) throws nbrArbitreInsufisantException {
		...
		// si la liste des match courant est nul alors tout les matchs du tour
		// ont été réalisé
		if (currentMatch.size() == 0) {
			...
			tableau.add(tmp);
			// si il n'y a qu'un seul vainqueur alors c'était le dernier match
			// de l'épreuve
	...
	/**
	 * Incrémente le score de l'équipe et du joueur dans le cadre d'une équipe
	 * individuelle
	 * 
	 * @param vainqueur
	 */
	...
	/**
	 * Méthode permettant de rentrer le score d'un match en vérifiant que c'est
	 * le bon arbitre
	 * 
	 * @param m
	 * @param se1
